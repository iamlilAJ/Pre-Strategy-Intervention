...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "iql")
    ...
    if config['ENV_KWARGS']['zero_index']:
        alg_name += ""
    else:
        alg_name +="far_landmark_"
...
def tune(default_config):
    ...
    alg_name = default_config.get("ALG_NAME", "iql_our_method")
    ...
    if default_config["RUN_BASELINE"] is True:
        name = "baseline_iql_np"
        seed_range = list(range(50))
    else:
        name = f"{alg_name}_{env_name}"
        seed_range = list(range(100))
    if default_config['ENV_KWARGS']['zero_index']:
        alg_name += ""
    else:
        alg_name +="far_landmark_"
