...
from jaxmarl.wrappers.noisy_wrapper import NoisyPrePolicyHanabiWrapper, ObsNoisyPrePolicyHanabiWrapper
...
def make_train(config, env):
    ...
    def train(rng):
        ...
        wrapped_env = CTRolloutManager(
            env, batch_size=config["NUM_ENVS"],
            preprocess_obs=False,
            if_augment_obs=config["ENV_KWARGS"]["if_augment_obs"],
        )
        # test env have no noise
        ...
        def _update_step(runner_state, unused):
            ...
            def _compute_targets(last_q, q_vals, reward, done):
                ...
                def _get_target(lambda_returns_and_next_q, rew_q_done):
                    ...
                    print(f"target_bootstrap: {target_bootstrap.shape}")
            ...
            if config["NUM_STEPS"] > 1:  # q-lambda returns
                ...
                lambda_targets = (
                    ...
                    + (1 - transitions.done[-1, :]) * config["GAMMA"] * last_q
                )
            ...
            def _learn_epoch(carry, _):
                ...
                def _learn_phase(carry, minibatch_and_target):
                    ...
                    def _loss_fn(params):
                        ...
                        agent_in = agent_in.reshape(
                             env.num_agents, -1, wrapped_env.obs_size
                        )
                        # agent_in = minibatch.obs
                        ...
                        print(f"q value in loss shape {q_vals.shape}")
                        ...
                        loss = jnp.mean((chosen_action_qvals - jax.lax.stop_gradient(target)) ** 2)
                ...
                targets = jax.random.permutation(_rng, lambda_targets, axis = 1)

                targets = targets.reshape(config["NUM_MINIBATCHES"], 2,-1)
            ...
            if config["WANDB_MODE"] != "disabled":
                ...
                def callback(metrics, original_seed):
                    ...
                    wandb.log(metrics, step=metrics["update_steps"].item())
        ...
        def get_greedy_metrics(rng, train_state):
            ...
            step_state, (_, dones, infos) = jax.lax.scan(
...
def single_run(config):
    ...
    alg_name += config["ENV_KWARGS"]["convention_type"]
...
def tune(default_config):
    ...
    default_config = {
        **default_config,
        **default_config["alg"],
    }  # merge the baselines config with the main config
    ...
    if default_config["RUN_BASELINE"]:
        alg_name += f"baseline_pqn_iql_{env_name}"
    else:
        alg_name = f"pre_policy_pqn_iql_{env_name}"
