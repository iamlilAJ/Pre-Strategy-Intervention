...
class HanabiEnv(HanabiGame):
    ...
    def __init__(
        ...
        num_moves=None,
        train_pre_policy=False,
        intrinsic_reward_ratio = 0.03,
        ...
        super().__init__(
            ...
            intervene_two_agents=intervene_two_agents
    ...
    def step_env(
        ...
        old_state = state
        new_state, reward, intrisnic_rewards = self.step_game(state, aidx, action)
    ...
    def get_obs(
        ...
        base_obs = jax.vmap(_observe)(self.agent_range)


        if self.train_pre_policy and self.if_augment_obs:
            print("Augment Observation")
            final_obs = {}
            for i, agent in enumerate(self.agents):

                if not self.intervene_two_agents:
                    if i==0:
                        reward_to_concat = new_state.last_intrinsic_reward[i]
                    else:
                        reward_to_concat = 0
                else:
                    reward_to_concat = new_state.last_intrinsic_reward[i]

                # Convert to a shape (1,) so we can concatenate
                reward_to_concat = jnp.array([reward_to_concat], dtype=jnp.float32)
                final_obs[agent] = jnp.concatenate([base_obs[i], reward_to_concat])

            return final_obs
        else:
            return {a: base_obs[i] for i, a in enumerate(self.agents)}
