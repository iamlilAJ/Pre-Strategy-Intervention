...
class State:
    ...
    # New field to store the last intrinsic reward
    last_intrinsic_reward: chex.Array
...
class HanabiGame(MultiAgentEnv):
    ...
    def __init__(
        ...
        color_map=["R", "Y", "G", "W", "B"],
        convention_type="the_chop",
        intervene_two_agents=False
    ):
        ...
        self.intervene_two_agents = intervene_two_agents

        assert convention_type in ["the_chop", "save_5"]
    ...
    def get_first_state(self, deck: chex.Array) -> State:
        ...
        state = State(
            ...
            turn=0,
            score=0,
            last_intrinsic_reward=jnp.zeros(self.num_agents, dtype='int32'), # reset intrinsic reward as 0.
        )
    ...
    def step_game(
        ...
        def _discard_play_fn(state: State, action: int):
            ...
            agent0_colors_revealed = state.colors_revealed[aidx]
            agent0_ranks_revealed = state.ranks_revealed[aidx]
            not_hinted_mask = jnp.logical_and(
                jnp.all(agent0_colors_revealed == 0, axis=1),
                jnp.all(agent0_ranks_revealed == 0, axis=1)
            )

            reversed_mask = not_hinted_mask[::-1]
            first_true = jnp.argmax(reversed_mask)
            rightmost_unhinted_card_idx = self.hand_size - 1 - first_true
            all_hinted = jnp.sum(not_hinted_mask) == 0

            # 3) Check if agent_0 discards the rightmost unhinted card
            is_rightmost_unhinted = (card_idx == rightmost_unhinted_card_idx)

            # 4) Assign intrinsic reward:
            #    - If all_hinted is True, no unhinted card remains, default to -1.
            #    - If discarding the rightmost unhinted card, +0.
            #    - Otherwise, -2 for discarding a different card.
            intrinsic_reward = jnp.where(
                is_discard,  # condition: agent is discarding
                jnp.where(
                    all_hinted,  # sub-condition: all cards are hinted
                    -1,  # if all hinted, no unhinted card => -1
                    jnp.where(is_rightmost_unhinted, 0, -2)  # else: 0 if correct, -2 otherwise
                ),
                -1  # if not discarding => -1
            ).astype(int)



            if self.convention_type != "the_chop":
                intrinsic_reward = 0

            # Create an array of intrinsic rewards, one for each agent
            # Initialize with zeros
            intrinsic_rewards = jnp.zeros(self.num_agents, dtype=jnp.int32)
            # Set the reward for the current agent
            intrinsic_rewards = intrinsic_rewards.at[aidx].set(intrinsic_reward)

            # only intervene single agent
            if self.intervene_two_agents:
                print("intervene two agents")
            else:
                intrinsic_rewards = intrinsic_rewards.at[1].set(0)
        ...
        def _hint_fn(state: State, action: int):
            ...
            is_agent_0 = (aidx == 0)  # only penalize agent_0

            # 1) find if the target player has unhinted 5
            target_hand = state.player_hands.at[hint_player].get()  # shape (hand_size, num_colors, num_ranks)
            rank_5_cards = jnp.any(target_hand[..., self.num_ranks - 1], axis=-1)
            # unhinted for that rank
            rank_5_unhinted_mask = jnp.logical_and(
                rank_5_cards,
                state.ranks_revealed[hint_player, :, self.num_ranks - 1] == 0
            )
            target_has_unhinted_5 = jnp.any(rank_5_unhinted_mask)
            ...
            if self.convention_type !="save_5":
                intrinsic_reward = 0
            ...
            # only intervene single agent
            if not self.intervene_two_agents:
                intrinsic_rewards = intrinsic_rewards.at[1].set(0)
        ...
        state, intrinsic_rewards = lax.cond(is_hint, _hint_fn, _discard_play_fn, state, action)
        ...
        return (
            state.replace(
                ...
                last_intrinsic_reward=intrinsic_rewards, # return intrinsic reward for observation concat
            ),
            reward,
            intrinsic_rewards,
        )
