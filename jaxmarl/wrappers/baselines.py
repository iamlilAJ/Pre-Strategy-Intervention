...
@struct.dataclass
class PrePolicyLogEnvState:
    env_state: State
    # For mixed/training returns
    episode_mixed_returns: jnp.ndarray
    episode_lengths: jnp.ndarray
    returned_episode_mixed_returns: jnp.ndarray
    returned_episode_lengths: jnp.ndarray

    # For original returns
    episode_extrinsic_returns: jnp.ndarray
    returned_episode_extrinsic_returns: jnp.ndarray


class PrePolicyWrapper(JaxMARLWrapper):

    def __init__(self, env: MultiAgentEnv, replace_info: bool = False):
        super().__init__(env)
        self.replace_info = replace_info

    # Removed @jax.jit to handle complex reward structures
    def reset(self, key: chex.PRNGKey) -> Tuple[Dict[str, chex.Array], PrePolicyLogEnvState]:
        obs, env_state = self._env.reset(key)
        state = PrePolicyLogEnvState(
            env_state,
            jnp.zeros((self._env.num_agents,)),
            jnp.zeros((self._env.num_agents,)),
            jnp.zeros((self._env.num_agents,)),
    ...
    # Updated step function without @jax.jit
    def step(
            self,
            key: chex.PRNGKey,
            state: PrePolicyLogEnvState,
            actions: Dict[str, Any],
    ) -> Tuple[Dict[str, chex.Array], PrePolicyLogEnvState, Dict[str, float], Dict[str, bool], Dict[str, Any]]:
        obs, env_state, reward, done, info = self._env.step(
            key, state.env_state, actions
        )

        # Initialize training rewards and original rewards
        mixed_rewards = {}
        extrinsic_rewards = {}
        training_rewards = {}
        ...
        for agent, (extrinsic_rew, intrinsic_rew) in reward.items():
            ...
            if self._env.all_agents_intrinsic:
                print("train all agents intrinsic")
        ...
        # Multiply training rewards by number of agents (as per on-policy codebase)
        # training_rewardlog = {agent: r * self._env.num_agents for agent, r in training_reward.items()}
        extrinsic_reward_log  = jax.tree_map(lambda x: x * self._env.num_agents, extrinsic_rewards)
        mixed_reward_log = jax.tree_map(lambda x: x * self._env.num_agents, mixed_rewards)  # As per on-policy codebase

        # Update episode returns and lengths
        ep_done = done["__all__"]
        mixed_new_episode_return = state.episode_mixed_returns + self._batchify_floats(mixed_reward_log)
        extrinsic_new_episode_return = state.episode_extrinsic_returns + self._batchify_floats(extrinsic_reward_log)
        new_episode_length = state.episode_lengths + 1
        state = PrePolicyLogEnvState(
            env_state=env_state,
            episode_mixed_returns=mixed_new_episode_return * (1 - ep_done),
            episode_lengths=new_episode_length * (1 - ep_done),
            returned_episode_mixed_returns=state.returned_episode_mixed_returns * (1 - ep_done)
                                     + mixed_new_episode_return * ep_done,
            returned_episode_lengths=state.returned_episode_lengths * (1 - ep_done)
                                     + new_episode_length * ep_done,

            episode_extrinsic_returns=extrinsic_new_episode_return * (1 - ep_done),
            returned_episode_extrinsic_returns=state.returned_episode_extrinsic_returns * (1 - ep_done)
                                    + extrinsic_new_episode_return * ep_done,
        )
        ...
        return obs, state, training_rewards, done, info



class PrePolicyHanabiWrapper(JaxMARLWrapper):

    def __init__(self, env: MultiAgentEnv, if_q_learning: bool = False, replace_info: bool = False):
        super().__init__(env)
        self.replace_info = replace_info
        self.if_q_learning = if_q_learning
    ...
    def step(
        ...
        if self._env.intervene_two_agents:
            ...
            training_rewards['__all__'] = (training_rewards['agent_0'] + training_rewards['agent_1']) / 2
        else :
            training_rewards['__all__'] = training_rewards['agent_0']

        # Update episode returns and lengths
        ep_done = done["__all__"]
        mixed_new_episode_return = state.episode_mixed_returns + self._batchify_floats(mixed_rewards)
        extrinsic_new_episode_return = state.episode_extrinsic_returns + self._batchify_floats(extrinsic_rewards)
        new_episode_length = state.episode_lengths + 1
...
class CTRolloutManager(JaxMARLWrapper):
    ...
    def __init__(self, env: MultiAgentEnv, batch_size:int, training_agents:List=None, preprocess_obs:bool=False,
